...
def wait_for_manual_verification(page, seconds=120):
    if not challenge_detected(page):
        return True
    st.warning(
        "Shutterstock requested verification. Complete the slider in the Chrome window. "
        "The system will continue automatically after verification."
    )
...
def upload_image(page, temp_path):
    ...
    controls = page.locator("button, [role='button'], label, [aria-label], [title]")
    keys = ("search by image", "image search", "camera", "upload image", "upload", "visual search")
    for i in range(min(controls.count(), 180)):
        el = controls.nth(i)
        try:
            attrs = [el.inner_text(timeout=250), el.get_attribute("aria-label"), el.get_attribute("title"), el.get_attribute("data-testid")]
            hay = " ".join(str(v) for v in attrs if v).lower()
        except Exception:
            continue
        if any(k in hay for k in keys) and set_file_via_click(page, el, temp_path):
            return True

    search = page.locator('input[placeholder*="Search for images" i], input[placeholder*="Search" i]').first
    if search.count():
        try:
            buttons = search.locator("xpath=ancestor::*[self::form or @role='search' or contains(translate(@class,'SEARCH','search'),'search')][1]//button")
            for i in reversed(range(min(buttons.count(), 12))):
                if set_file_via_click(page, buttons.nth(i), temp_path):
                    return True
        except Exception:
            pass
    return False
...
def check_shutterstock(page, upload_bytes):
    ...
    try:
        ...
        if not wait_for_manual_verification(page):
            return "MANUAL CHECK", "", 0.0, "Verification was not completed within 120 seconds."

        dismiss_overlays(page)

        if not upload_image(page, temp_path):
            return "AUTOMATION ERROR", "", 0.0, "Could not trigger Shutterstock Search by Image."
        ...
        if not wait_for_manual_verification(page):
            return "MANUAL CHECK", "", 0.0, "Verification was requested after upload and was not completed."

        candidates = collect_candidates(page)
        if not candidates:
            return "NOT FOUND", "", 0.0, "No Shutterstock result cards detected."
        ...
        if best_score >= MATCH_THRESHOLD:
            return "MATCH FOUND", best_url, best_score, f"Compared {len(candidates)} Shutterstock results."
        if best_score >= POSSIBLE_THRESHOLD:
            return "POSSIBLE MATCH", best_url, best_score, f"Compared {len(candidates)} Shutterstock results."
        return "NOT FOUND", "", best_score, f"Compared {len(candidates)} Shutterstock results."
    finally:
...
st.caption("Runs Shutterstock checks through your own visible Chrome browser and keeps the browser profile between runs.")
st.success("Local mode uses your own connection. If Shutterstock asks for verification, solve it once in the opened Chrome window; the app then continues automatically.")
...
if st.button("Start Check", type="primary", use_container_width=True):
    if os.name != "nt":
        st.error("This local version is intended for Windows. Run it on your PC with Python 3.12.")
        st.stop()
    ...
    for i, row in enumerate(results, start=1):
        if "_preview" not in row:
            continue
        with st.expander(f"{i}. {row['position']} — {row['status']}", expanded=row["status"] in ("MATCH FOUND", "POSSIBLE MATCH", "MANUAL CHECK")):
            c1, c2 = st.columns(2)
            with c1:
